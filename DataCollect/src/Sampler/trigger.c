 ...
 * @param bladerf_trigger *master_trig -- trigger associated with master device
 * @param bladerf_trigger *slave_trig -- trigger associated with slave device
...
int trigger_init(struct bladerf *master_dev, struct bladerf *slave_dev, bladerf_trigger *master_trig, bladerf_trigger *slave_trig){
	...
	/* init slave trigger */
	blader_trigger_init(slave_dev, channel, signal, slave_trig);
	if(status != 0){
 ...
 * @param bladerf_trigger *master_trig -- master trigger
